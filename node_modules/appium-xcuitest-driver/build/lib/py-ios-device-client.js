...
class Pyidevice {
    ...
    async execute(args, opts = {}) {
        ...
        const cmdStr = support_1.util.quote([/** @type {string} */ (this.binaryPath), ...finalArgs]);
        logger_1.default.debug(`Executing ${cmdStr}`);
        try {
            if (asynchronous) {
                const result = new teen_process_1.SubProcess(/** @type {string} */ (this.binaryPath), finalArgs, { cwd });
                await result.start(0);
                return result;
            }
            const result = await (0, teen_process_1.exec)(/** @type {string} */ (this.binaryPath), finalArgs, { cwd });
            if (logStdout) {
