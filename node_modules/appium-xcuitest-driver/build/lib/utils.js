...
async function getAndCheckXcodeVersion() {
    let version;
    try {
        version = await appium_xcode_1.default.getVersion(true);
    }
    catch (err) {
        logger_1.default.debug(err);
        logger_1.default.errorAndThrow(`Could not determine Xcode version: ${err.message}`);
    }
    // we do not support Xcodes < 7.3,
    if (version.versionFloat < 7.3) {
        logger_1.default.errorAndThrow(`Xcode version '${version.versionString}'. Support for ` +
...
async function markSystemFilesForCleanup(wda) {
    ...
    const logsRoot = path_1.default.resolve(await wda.retrieveDerivedDataPath(), 'Logs');
    let markersCount = 0;
...
async function clearSystemFiles(wda) {
    ...
    const globPattern = `${os_1.default.tmpdir()}/${XCTEST_LOGS_CACHE_FOLDER_PREFIX}*/`;
    ...
    if (lodash_1.default.isEmpty(dstFolders)) {
        ...
        for (const dstFolder of dstFolders) {
            ...
            bluebird_1.default.resolve(support_1.fs.walkDir(dstFolder, true, (itemPath, isDir) => {
                ...
                const fileName = path_1.default.basename(itemPath);
...
async function getDriverInfo() {
    const stat = await support_1.fs.stat(path_1.default.resolve(__dirname, '..'));
...
function isLocalHost(urlString) {
    try {
        const hostname = /** @type {string} */ (url_1.default.parse(urlString).hostname);
