...
describe('Android app user - Continue as a Guest user to view All Games draw data for QUICK DRAW Game', function () {
    
     // Retry 1 time if test fails
     let count =0;
     this.retries(1);

     const testID = 'https://rosedigital.atlassian.net/browse/MRMNYL-201';

     
     it('Verify Android app user is able to view All games draw data in QUICK DRAW screen', async function () {  
        ...
        allureReporter.addDescription(`Description: Guest user can view All games draw data in QUICK DRAW game Screen\n\n TestID: ${testID}`);
        ...
        // 7. swipe until QUICK DRAW game and tap
        allureReporter.addStep('Swipe till QUICK DRAW game and tap on QUICK DRAW');
        (await QuickDrawGamePage.QuickDrawGame).click();

        // 8. assert for Winning numbers, draw details
        await expect(await QuickDrawGamePage.quickDraw_WinningNumbers_DrawNumbers).toExist({message: "Winning data is not available"});
        
        allureReporter.addStep('Verified Winning and Drawing details for QUICK DRAW game');
    });
});
