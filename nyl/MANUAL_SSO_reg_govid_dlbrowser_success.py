...
# registration with OTP pass and real Driver's License on DESKTOP BROWSER flow
# Prior to running test, image files need to be saved and available on local machine
# Images needed are -
# front of drivers' license in landscape orientation,
# back of drivers' license in landscape orientation, and
# image of users' face in portrait orientation

class NYlotto(confTest.NYlottoBASE):

    def test_regDLBrowserSuccess(self):
        # Jira test ticket - https://rosedigital.atlassian.net/browse/NYL-2401
        testemail = self.testemail
        testenv = self.env
        print("TESTING " + testenv + " ENVIRONMENT")
        print("\nChecks successful registration with OTP pass and real Driver's License on Browser method")
        print('\n----------\n' + 'Test setup')
        ...
        try:
            driver.execute_script('alert("Test will pause for max of 15 minutes while tester uploads images for '
                                  'front of drivers license, back of drivers license, and '
                                  'image of face for gov id verification.  '
                                  'STOP interacting with test once last image is SUBMITTED. '
        ...
        # wait for GOOGLE SEARCH button to appear on screen
        ...
        wait.until(EC.element_to_be_clickable((By.XPATH, var.resetPswV.gSearchButton[1])))
        # Last screen. Successful registration would redirect to Google.com.
